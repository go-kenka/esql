...
func TestBuilder(t *testing.T) {
	tests := []struct {
		...
		{
			input: NewBuilder(Postgres).CreateTable("users").
				Columns(
		...
		{
			input: NewBuilder(Postgres).AlterTable("users").
				AddColumn(Column("group_id").Type("int").Attr("UNIQUE")).
		...
		{
			input: NewBuilder(Postgres).AlterIndex("old").
				Rename("new"),
		...
		{
			input:     NewBuilder(Postgres).Insert("users").Columns("age").Values(1),
			wantQuery: `INSERT INTO "users" ("age") VALUES ($1)`,
			wantArgs:  []any{1},
		},
		{
			input:     NewBuilder(Postgres).Insert("users").Columns("age").Values(1).Schema("mydb"),
			wantQuery: `INSERT INTO "mydb"."users" ("age") VALUES ($1)`,
			wantArgs:  []any{1},
		},
		{
			input:     NewBuilder(SQLite).Insert("users").Columns("age").Values(1).Schema("mydb"),
			wantQuery: "INSERT INTO `users` (`age`) VALUES (?)",
			wantArgs:  []any{1},
		},
		{
			input:     NewBuilder(Postgres).Insert("users").Columns("age").Values(1).Returning("id"),
			wantQuery: `INSERT INTO "users" ("age") VALUES ($1) RETURNING "id"`,
			wantArgs:  []any{1},
		},
		{
			input:     NewBuilder(Postgres).Insert("users").Columns("age").Values(1).Returning("id").Returning("name"),
		...
		{
			input:     NewBuilder(Postgres).Insert("users").Columns("name", "age").Values("a8m", 10),
		...
		{
			input:     NewBuilder(Postgres).Insert("users").Columns("name", "age").Values("a8m", 10).Values("foo", 20),
			wantQuery: `INSERT INTO "users" ("name", "age") VALUES ($1, $2), ($3, $4)`,
			wantArgs:  []any{"a8m", 10, "foo", 20},
		},
		{
			input: NewBuilder(Postgres).Insert("users").
				Columns("name", "age").
				Values("a8m", 10).
				Values("foo", 20).
				Values("bar", 30),
		...
		{
			input:     NewBuilder(Postgres).Update("users").Set("name", "foo"),
			wantQuery: `UPDATE "users" SET "name" = $1`,
			wantArgs:  []any{"foo"},
		},
		{
			input:     NewBuilder(Postgres).Update("users").Set("name", "foo").Returning("*"),
			wantQuery: `UPDATE "users" SET "name" = $1 RETURNING *`,
			wantArgs:  []any{"foo"},
		},
		{
			input:     NewBuilder(Postgres).Update("users").Set("name", "foo").Returning("id", "name"),
			wantQuery: `UPDATE "users" SET "name" = $1 RETURNING "id", "name"`,
			wantArgs:  []any{"foo"},
		},
		{
			input:     NewBuilder(Postgres).Update("users").Set("name", "foo").Schema("mydb"),
			wantQuery: `UPDATE "mydb"."users" SET "name" = $1`,
			wantArgs:  []any{"foo"},
		},
		{
			input:     NewBuilder(SQLite).Update("users").Set("name", "foo").Schema("mydb"),
			wantQuery: "UPDATE `users` SET `name` = ?",
			wantArgs:  []any{"foo"},
		},
		{
		...
		{
			input:     NewBuilder(SQLite).Update("users").Set("name", "foo").Returning("id", "name").OrderBy("name").Limit(10),
			wantQuery: "UPDATE `users` SET `name` = ? RETURNING `id`, `name` ORDER BY `name` LIMIT 10",
			wantArgs:  []any{"foo"},
		},
		{
			input:     NewBuilder(Postgres).Update("users").Set("name", "foo").Set("age", 10),
			wantQuery: `UPDATE "users" SET "name" = $1, "age" = $2`,
			wantArgs:  []any{"foo", 10},
		},
		{
			input: NewBuilder(Postgres).Update("users").
				Set("active", false).
		...
		{
			input:     NewBuilder(Postgres).Update("users").Set("name", "foo").Where(EQ("name", "bar")),
		...
		{
			input:     NewBuilder(Postgres).Update("users").Set("name", "foo").SetNull("spouse_id"),
		...
		{
			input:     NewBuilder(Postgres).Select().From(Table("users")),
			wantQuery: `SELECT * FROM "users"`,
		},
		{
		...
		{
			input:     NewBuilder(Postgres).Select().From(Table("users").Unquote()),
			wantQuery: "SELECT * FROM users",
		},
		{
		...
		{
			input:     NewBuilder(Postgres).Select().From(Table("users").As("u")),
			wantQuery: `SELECT * FROM "users" AS "u"`,
		},
		{
			input: func() Querier {
		...
		},
		{
			input: func() Querier {
				t1 := Table("users").As("u")
				t2 := Table("groups").As("g")
				return NewBuilder(Postgres).Select(t1.C("id"), t2.C("name")).From(t1).Join(t2)
			}(),
			wantQuery: `SELECT "u"."id", "g"."name" FROM "users" AS "u" JOIN "groups" AS "g"`,
		},
		{
...
func TestSelector_OrderByExpr(t *testing.T) {
	...
	query, args = NewBuilder(Postgres).
		Select("*").
		From(Table("users")).
		Where(GT("age", 28)).
		OrderBy("name").
...
func TestBuilderContext(t *testing.T) {
	...
	sel := NewBuilder(Postgres).Select().WithContext(ctx)
...
func (p point) FormatParam(placeholder string, info *StmtInfo) string {
	require.Equal(p.T, MySQL, info.Dialect)
...
func TestReusePredicates(t *testing.T) {
	...
	for _, tt := range tests {
		query, args := NewBuilder(Postgres).Select().From(Table("users")).Where(tt.p).Query()
		require.Equal(t, tt.wantQuery, query)
		require.Equal(t, tt.wantArgs, args)
		query, args = NewBuilder(Postgres).Select().From(Table("users")).Where(tt.p).Query()
		require.Equal(t, tt.wantQuery, query)
...
func TestUpdateBuilder_OrderBy(t *testing.T) {
	u := NewBuilder(MySQL).Update("users").Set("id", Expr("`id` + 1")).OrderBy("id")
	require.NoError(t, u.Err())
	query, args := u.Query()
	require.Nil(t, args)
	require.Equal(t, "UPDATE `users` SET `id` = `id` + 1 ORDER BY `id`", query)

	u = NewBuilder(Postgres).Update("users").Set("id", Expr("id + 1")).OrderBy("id")
