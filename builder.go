...
// package esql provides wrappers around the standard database/sql package
...
// Columns appends the a list of columns to the
...
func (u *UpdateSet) Table() *SelectTable {
	return NewBuilder(u.update.dialect).Table(u.update.table)
}
...
func (u *UpdateSet) SetExcluded(name string) *UpdateSet {
	...
	default:
		t := NewBuilder(u.update.dialect).Table("excluded")
...
func (i *InsertBuilder) writeConflict(b *Builder) {
	...
	u := &UpdateSet{columns: i.columns, update: NewBuilder(i.dialect).Update(i.table)}
	u.update.Builder = *b
...
// after Add(c) will erase previous calls with c from the
...
type Builder struct {
	sb        *strings.Builder // underlying
	dialect   string           // configured
...
// on the configured  It defaults to "`".
...
// WriteOp writes an operator to the
...
type (
	...
	ParamFormatter interface {
		...
		// from '?' to 'ST_GeomFromWKB(?)' for MySQL
		FormatParam(placeholder string, info *StmtInfo) string
	}
)

// Arg appends an input argument to the
...
// Args appends a list of arguments to the
...
// Join joins a list of Queries to the
...
// SetDialect sets the builder  It's used for garnering dialect specific queries.
func (b *Builder) SetDialect(dialect string) {
	b.dialect = dialect
}

// Dialect returns the dialect of the
...
// clone returns a shallow clone of a
...
func NewBuilder(name string) *DialectBuilder {
	return &DialectBuilder{name}
}

// Describe creates a DescribeBuilder for the configured
//
//	Dialect(Postgres).
//		Describe("users")
...
// CreateTable creates a TableBuilder for the configured
//
//	Dialect(Postgres).
//		CreateTable("users").
//			Columns(
...
// AlterTable creates a TableAlter for the configured
//
//	Dialect(Postgres).
//		AlterTable("users").
//		AddColumn(Column("group_id").Type("int").Attr("UNIQUE")).
...
// AlterIndex creates an IndexAlter for the configured
//
//	Dialect(Postgres).
//		AlterIndex("old").
//		Rename("new")
...
// Column creates a ColumnBuilder for the configured
//
//	Dialect(Postgres)..
...
// Insert creates a InsertBuilder for the configured
//
//	Dialect(Postgres).
...
// Update creates a UpdateBuilder for the configured
//
//	Dialect(Postgres).
...
// Delete creates a DeleteBuilder for the configured
//
//	Dialect(Postgres).
//		Delete().From("users")
...
// Select creates a Selector for the configured
//
//	Dialect(Postgres).
...
// Table creates a SelectTable for the configured
//
//	Dialect(Postgres).
//		Table("users").As("u")
...
// With creates a WithBuilder for the configured
//
//	Dialect(Postgres).
//		With("users_view").
...
// CreateIndex creates a IndexBuilder for the configured
//
//	Dialect(Postgres).
...
// DropIndex creates a DropIndexBuilder for the configured
//
//	Dialect(Postgres).
//		DropIndex("name")
